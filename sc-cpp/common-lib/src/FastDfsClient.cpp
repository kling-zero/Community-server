...
void FastDfsClient::init()
{
	...
	// 初始化fastfds客户端
	int result = fdfs_client_init(m_strConfigPath.c_str());
	if (result != 0)
	{
		isInit = false;
...
FastDfsClient::FastDfsClient(const char* fdsConfig, int logLevel)
{
	//初始化成员变量
	m_strConfigPath = fdsConfig;
...
std::string FastDfsClient::uploadFile(const std::string& fileName)
{
	...
	size_t size = (size_t)fsRead.tellg();
...
std::string FastDfsClient::downloadFile(const std::string& fieldName, std::string& savePath)
{
	...
	strcpy(local_filename, fileName.c_str());
	...
	//下载文件
	UINT32 nRet = m_func_DownloadFileByID((BYTE*)szFileID, byFileBuff, &nFileSize);
	if (nRet != enumSuccess_FDFS)
	{
...
bool FastDfsClient::deleteFile(const std::string& fieldName)
{
	...
	//删除文件
	UINT32 nRet = m_func_DeleteFileByID((BYTE*)szFileID);
	if (nRet != enumSuccess_FDFS)
	{
