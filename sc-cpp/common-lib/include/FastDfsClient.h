...
public:
	...
	// Returns:   
	// Qualifier: 构造初始化
	// Parameter: const char * fdsConfig FastDFS配置文件路劲，比如:/etc/fdfs/client.conf
	// Parameter: int logLevel 日志等级，采用的是unix日志等级
	//  0: LOG_EMERG
	//	1: LOG_ALERT
	//	2: LOG_CRIT
	//	3: LOG_ERR
	...
	// Qualifier: 上传文件
	// Parameter: std::string & fileName 包含路径的上传文件名
	//************************************
	...
	// Qualifier: 下载文件
	// Parameter: std::string & fieldName fastdfs对应的文件名，包含：组名+文件名
