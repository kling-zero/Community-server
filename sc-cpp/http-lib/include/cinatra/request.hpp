...
namespace cinatra {
	...
	public:
		nlohmann::json getUserPayload() const { return userPayload; }
		void setUserPayload(nlohmann::json val) { userPayload = val; }
		...
		//补充：获取/*之后的所有参数值
		std::vector<std::string_view> get_query_values() {
			...
			for (auto item : memento::pathinfo_mem) {
				...
				int n = -1;
				//定义map_url用于对url进行解码
				auto map_url = url.length() > 1 && tail == 1 ? url.substr(0, url.length() - 1) : url;
				for (auto param : params)
				{
					n++;
					if (code_utils::is_url_encode(param))
					{
						std::string map_key = std::string(map_url.data(), map_url.size()) + std::to_string(n);
						auto ret = utf8_character_pathinfo_params_.emplace(map_key, code_utils::get_string_by_urldecode(param));
						result.push_back(std::string_view(ret.first->second.data(), ret.first->second.size()));
						continue;
					}
