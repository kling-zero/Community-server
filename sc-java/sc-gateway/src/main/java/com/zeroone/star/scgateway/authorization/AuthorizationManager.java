package com.zeroone.star.scgateway.authorization;
...
public class AuthorizationManager implements ReactiveAuthorizationManager<AuthorizationContext> {
    ...
    private List<String> queryRoleListByPath(String path) {
        ...
        if (obj == null) {
            log.info("AuthorizationManager#queryRoleListByPath, redis get null object");
            return new ArrayList<>();
        }

        List<String> authorities = Convert.toList(String.class, obj);
        authorities = authorities.stream()
                .map(roleName -> roleName = AuthConstant.AUTHORITY_PREFIX + roleName)
